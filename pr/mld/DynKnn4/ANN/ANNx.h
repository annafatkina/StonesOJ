...
public:
	...
	float dist(ANNpoint q) const	// (squared) distance from q
	{
		switch (ANN::MetricType)
		{
		case ANN_METRIC0:
			return  (float)ANN_POW0(q[cd] - cv);
		case ANN_METRIC1:
			return  (float)ANN_POW1(q[cd] - cv);
		case ANN_METRIC2:
			return  (float)ANN_POW(q[cd] - cv);
		case ANN_METRICP:
			return  (float)ANN_POWp(q[cd] - cv);
		}
		return (float)ANN_POW(q[cd] - cv);
	}

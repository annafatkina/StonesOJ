 ...
 *  \brief  COMPUTES THE LINE WITH THE BEST FIT TO A SET OF 2D POINTS AND ERASE DISTANT POINTS
  TAKING INTO ACCOUNT THE DISTORTION MODEL
 *  \pre Any parameter can be null.
 *  \param [in] a INPUT POLINOMIAL DISTORTION MODEL
 *  \param [in] Na INPUT POLINOMIAL DISTORTION MODEL DEGREE
 *  \param [in]  xc, yc INPUT CENTER OF THE DISTORTION MODEL
  ...
  int *Na) // OUTPUT DEGREE OF POLINOMIAL DISTORTION MODEL (Na=4)
{
  ...
  *a=(double*) malloc(sizeof(double)*(*Na+1));
  ...
  /* WE NORMALIZE THE POINT COORDINATES */
  factor_n=0.; cont=0;
  for(m=0;m<Nl;m++){
  ...
  /* WE CALL TO THE ZOOM NORMALIZATION */
...
/**************************************************************************************
AUXILIARY FUNCTION ami_inverse_lens_distortion()
**************************************************************************************/

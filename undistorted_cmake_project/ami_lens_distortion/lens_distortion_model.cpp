...
/**
 * \fn point2d<double> lens_distortion_model::
        evaluation_quotient(const point2d<double> &p) const
 * \brief COMPUTE THE MODEL IN A POINT (WE CORRECT THE DISTORTION)
 * \param p point to be evaluated
 * \author Luis Alvarez
 */
 // COMPUTE THE MODEL IN A POINT
AMI_DLL_CPP point2d<double> lens_distortion_model::
  evaluation_quotient(const point2d<double> &p) const
 {
 ...
 /**
 * \fn point2d<double> lens_distortion_model::
        evaluation_quotient(point2d<double> &p)
 * \brief COMPUTE THE MODEL IN A POINT (WE CORRECT THE DISTORTION)
 * \param p point to be evaluated
 * \author Luis Alvarez
 */
...
/**
 * \fn point2d<double> lens_distortion_model::
        inverse_evaluation(point2d<double> &p)
...
/**
 * \fn point2d<double> lens_distortion_model::
        inverse_evaluation_quotient(point2d<double> &p)
 * \brief COMPUTE THE INVERSE RATIONAL MODEL IN A POINT
 * \param p point to be evaluated
 * \author Luis Alvarez, Daniel Santana-Cedres
 */
...
AMI_DLL_CPP point2d<double> lens_distortion_model::
  inverse_evaluation_quotient(point2d<double> &p){
    if(d.size()==2){
      ...
      //We show a message if the discriminant is negative and return the input
      //point
      if(discriminant<0)
      {
          //cout << "It's no possible to compute the inverse_evaluation_quotient:\
          //        discriminant<0" << endl;
          return p;
      }
      ...
      if(Nr==0){
        ...
        //point
        //cout << "It's no possible to compute the inverse_evaluation_quotient:\
        //        Problem with polynomial roots<0" << endl;
        return p;
      }
...
/**
 * \fn point2d<double> lens_distortion_model::
      inverse_evaluation_fast(point2d<double> &p,double dl1r, double *a, int Na)
 * \brief COMPUTE THE INVERSE MODEL IN A POINT
 * \param p point to be evaluated
 *  \param [in] dl1r coeficient interpolated from vector of max distorsion d
                istances
...
point2d<double> lens_distortion_model::
  inverse_evaluation_fast(point2d<double> &p,double dl1r, double *a, int Na){
 double x,y;
...
/**
 * \fn point2d<double> lens_distortion_model::
        inverse_evaluation(const point2d<double> &p) const
 * \brief COMPUTE THE INVERSE MODEL IN A POINT
 * \param p
 * \author Luis Alvarez
 */
...
/**
 * \fn point2d<double> lens_distortion_model::
        inverse_evaluation_quotient(const point2d<double> &p) const
 * \brief COMPUTE THE INVERSE RATIONAL MODEL IN A POINT
 * \param p
 * \author Luis Alvarez, Daniel Santana-Cedres
 */

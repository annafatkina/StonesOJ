...
/* *
* \page code_image.h \link image.h \endlink
* \brief Code of  \ref code_image.h "image.h"
* \includelineno image.h
* \brief \b Code: \n
* - \ref code_image.h "image.h"
*/
...
#include <stdio.h>
#include <stdlib.h>
#include "../ami_image/io_bmp/ami_bmp.h"
...
namespace ami {
	...
	*/
	...
	class image{
		...
		std::vector <int> roi_ /** ROI : Region Of Interest =(x0,x1,y0,y1,c0,c1) subwindow corners used (if defined) to operate only in a subwindow area */;
	...
	private:
		int read_with_ami_bmp (std::string name);
		int write_with_ami_bmp (std::string name);
	...
	* \brief method to downsample the image by a factor of 2. We filter the image before downsampling
	*/
	template<class T>
...
#pragma omp parallel for shared(i_end,n0,n_s,sampled_img,width_s)
			for(int i=1;i<i_end;i++){
				...
				for(int j=1;j<j_end;j++){
					int m=m0+2*j;
					sampled_img[m_s+j]=0.25*image_[m]+0.125*(image_[m+1]+image_[m-1]+image_[m+width_]+image_[m-width_])+
						0.0625*(image_[m+width_+1]+image_[m+width_-1]+image_[m-width_+1]+image_[m-width_-1]);
				}
			}
		}
		...
		for(int n=0;n<nChannels_;n++){
			...
			for(int i=0;i<height_s;i++){
				...
				for(int j=0;j<width_s;j+=width_s-1){
					...
					sampled_img[m_s+j]=0.25*image_[m]+0.125*(image_[mN]+image_[mS]+image_[mW]+image_[mE])+
						0.0625*(image_[mNW]+image_[mNE]+image_[mSW]+image_[mSE]);
				}
			}
		}
	...
	/**
	* \fn template <class T> template <class U> void image<T>::get_histogram(std::vector<U> &histograma, int channel)
	* \brief Get histogram of a channel of image
	* \author Carlos Falcon
	*/
	template <class T> template <class U>
	void image<T>::get_histogram(std::vector<U> &histograma/** histograma de dimension 256*/,
...
#pragma omp parallel for
	...
	/**
	* \fn template <class T> void image<T>::init(const int width,const int height, const int nChannels)
	...
	*/
	...
	//////////////////////////////////////////////////////////////////////////////////////////////
	/**
	* \fn template <class T> image<T>::image(const T *vector,const int width,const int height,const int channels)
	...
	*/
	...
	image<T>::image(cv::Mat image) {
		width_ = image.cols;
		height_ = image.rows;
		nChannels_ = image.channels();

		std::vector<cv::Mat> bgr(nChannels_);
		cv::split(image, bgr);
...
#pragma omp for nowait
			...
			{
				image_[i + n2_] = bgr[n].data[i];
			}
		}
	}

	template <class T>
	cv::Mat image<T>::GetMat() {
		cv::Mat result;
...
#pragma omp for nowait
			{
				bgr[n] = cv::Mat(height(), width(), CV_8UC1, &image_[n2_]);
			}
		}
		cv::merge(bgr, result);
	...
	template <class T>
	image<T>::image(const T *vector,const int width,const int height,const int channels)
	{
		width_=width ;
		height_=height;
		nChannels_=channels;
...
#pragma omp for nowait
	...
	//////////////////////////////////////////////////////////////////////////////////////////////
	/**
	* \fn template <class T> template<class U> void image<T>::hsv_to_rgb(image<U> &image)
	...
	*/
	...
	//////////////////////////////////////////////////////////////////////////////////////////////
	/**
	* \fn template <class T> void image<T>::rgb_to_hsv(image<unsigned char> &H,image<unsigned char> &S, image<unsigned char> &V)
	* \brief function to get colour model HSV from RGB (unsigned char version)
	* \author Carlos Falcon
	*/
	template <class T>
	void image<T>::rgb_to_hsv(image<unsigned char> &H,image<unsigned char> &S, image<unsigned char> &V)
	{
		if (nChannels_!=3) return;
		...
		for (int i=0; i<size_;i++)
		{
			...
			//H
			if(rgb_max==red){
				H[i]=(unsigned char) ((int) 0+43*((int) green - blue)/(rgb_max - rgb_min));
			}
			else if (rgb_max == green) {
				H[i]= (unsigned char) ((int) 85+ 43*((int) blue-red)/(rgb_max - rgb_min));
			}
			else{
				H[i]= (unsigned char) ((int) 171 + 43*((int) red - green)/(rgb_max - rgb_min));
			}
		}
	}
	...
	//////////////////////////////////////////////////////////////////////////////////////////////
	/**
	* \fn template <class T> void image<T>::set_roi(const int x0,const int x1,const int y0,const int y1)
	...
	*/
	...
	void image<T>::set_roi(const int x0,const int x1,const int y0,
		const int y1,const int c0, const int c1)
	{
		if(x0>x1 || y0>y1 || x0>width_ || y0>height_ || c0>nChannels_ || c0>c1 ) return;
	...
	*/
	...
	/////////////////////////////////////////////////////////////////////////////////////////////
	/**
	* \fn template <class T> template <class U> void  image<T>::set_roi_image(const image<U> &image2)
	* \brief function to fill the image with a subwindow image
	* \author Luis Alvarez
	*/

	template <class T> template <class U> void  image<T>::set_roi_image(const image<U> &image2)
	{
		...
		if( x1<=width_ &&  y1<=height_ && x0<=x1 && y0<=y1 && c1<=nChannels_ && c0<=c1){
			int width=x1-x0;
			int height=y1-y0;
			int size=width*height;

			if(image2.width()!=width || image2.height()!=height || image2.nChannels()!=nChannels_) return;
...
#pragma omp for nowait
	...
	//////////////////////////////////////////////////////////////////////////////////////////////
	/**
	* \fn template <class T> image<T>  const image<T>::get_roi_image(const std::vector <int> &roi2) const
...
#pragma omp for nowait
	...
	//////////////////////////////////////////////////////////////////////////////////////////////
	/**
	* \fn template <class T> template <class U> void  image<T>::get_roi_image(image<U> &image2) const
	* \brief function to fill a subwindow image
	* \author Luis Alvarez
	*/

	template <class T> template <class U> void  image<T>::get_roi_image(image<U> &image2) const
	{
		int x0,x1,y0,y1,c0,c1;
		...
		if( x1<= width_ &&  y1<= height_ && x0<=x1 && y0<=y1 &&  c1<= nChannels_ && c0<=c1){
			...
			if(image2.width()!=width || image2.height()!=height || image2.nChannels()!=nChannels_){
...
#pragma omp for nowait
	...
	*/
	...
	{
		if( width_!=image2.width() || height_!=image2.height() || nChannels_!=image2.nChannels()){
			width_=image2.width();
...
#pragma omp parallel for shared(k_end) private(k)
	...
	* \brief function to apply a linear transform given by an homography H to the image. H goes to the output image to the input image
	* \author Luis Alvarez
	*/
	template <class T>
...
#pragma omp parallel for shared(H,image2,n,n0,width_1,height_1)
	...
	//////////////////////////////////////////////////////////////////////////////////////////////
	/**
	* \fn template <class T> image<T> const image<T>::resize(const int width, const int height) const
...
#pragma omp parallel for shared(image2,n,n0,n2,scale_x,scale_y)
		...
		for(int n=0;n<nChannels_;n++){
			int n0=n*width_*height_;
			int n2=n*width*height;
#pragma omp parallel for shared(image2,n,n0,n2,scale_x,scale_y,width_1,height_1)
			for(int i=0;i<height;i++){
				double y0=scale_y*i;
				int y0i=y0;
				double dy0=y0-y0i;
	...
	//////////////////////////////////////////////////////////////////////////////////////////////
	/**
	* \fn template <class T> image<T> const image<T>::resize_no_omp(const int width, const int height) const
	...
	*/
	...
	image<T> const image<T>::resize_no_omp(const int width, const int height) const
	{
		...
		if(width<=width_/2 && height<=height_/2) return(((*this).sampling()).resize_no_omp(width,height));
...
#pragma omp parallel for shared(size,aux)
	...
	/**
	* \fn int image<T>::write_with_ami_bmp (std::string name)
	* \brief Function to write an image to disk with ami_bmp.
	* \param [in] name Filename of the image.
	* \author Jesus Rodriguez
	*/
	template <class T>
	int image<T>::write_with_ami_bmp (std::string name/** image file name */)
	{
		// BUILD NAME
		char name_c[200];
		snprintf(name_c, 199, "%s", name.c_str());
		// ALLOC MEMORY
		int size_ = width_ * height_;
		unsigned char *red, *green, *blue;
		red   = new unsigned char[size_];
		...
		int mb = 2*size_;
		// REFILL DATA VECTORS
		if(nChannels_ < 1) return 0;
		if(nChannels_ > 2)
		{
			for(long i=height_-1;i>-1;i--)
			{
				int m = (height_-1-i)*width_;
				for(long j=0;j<width_;j++,mr++,mg++,mb++,m++)
				{
					red[m]  = (unsigned char)image_[mr];//R
					green[m]= (unsigned char)image_[mg];//G
					blue[m] = (unsigned char)image_[mb];//B
				}
			}
		}
		else
		{
			...
			{
				int m = (height_-1-i)*width_;
				for(long j=0;j<width_;j++,mr++,m++)
				{
					red[m]  = green[m]= blue[m] = (unsigned char)image_[mr];
				}
			}
		}
		// WRITE IMAGE
		ami_write_bmp(name_c, red, green, blue, width_, height_);
		// FREE MEMORY
		delete []red;
		delete []green;
		delete []blue;
	...
	*/
	...
	int image<T>::write(std::string name /** image file name */)
	{
		...
		if( (extension == std::string("bmp")) || (extension == std::string("BMP")))
		{
			return write_with_ami_bmp(name);
		}
	...
	T& image<T>::operator()(const int &x,const int &y,const int &channel){
		...
		if((x+y*width_+channel*width_*height_)>=(image_.size())){
			...
			//printf("image size()=%d index to accces the vector =%d\n",
				image_.size(),x+y*width_+channel*width_*height_);
			int j; scanf("%d",&j);
			exit(0);
		}
#endif
	...
	*/
	...
	//////////////////////////////////////////////////////////////////////////////////////////////
	/**
	* \fn image<T>::image(const int width,const int height, const T &a, const T &b, const T &c)
	* \brief image constructor taking memory and initialiting the channels: R with a value, G with b value and B with c value
	* \author Carlos Falcon
	*/
	template <class T>
	image<T>::image(const int width,const int height, const T &a, const T &b, const T &c)
	{
		width_=width ;
		height_=height;
		nChannels_=3;
...
#pragma omp parallel for shared(size) private(i)
	...
	//////////////////////////////////////////////////////////////////////////////////////////////
	/**
	* \fn image<T>::image(const int width,const int height, const int nChannels, const T &a)
...
#pragma omp parallel for shared(size) private(i)
	...
	//////////////////////////////////////////////////////////////////////////////////////////////
	/**
	* \fn template <class T> image<T>::image(const int width,const int height, const int nChannels)
	* \brief image constructor taking memory and initialiting with a value
	* \author Luis Alvarez
	*/
	template <class T>
	...
	*/
	...
	{
		//#pragma omp parallel for
		for(int k=0; k<nChannels_;k++){                                                                     // all of channels
			for(int i=0; i< height_;i++){                                                                   // reading for columns
				for(int j=0; j<(width_/2);j++){                                                             // reading for rows
					T aux = image_[(k*height_*width_)+i*width_+j];                                          //
					image_[(k*height_*width_)+i*width_+j] = image_[(k*height_*width_)+i*width_+width_-j-1]; // swapping mirrored values in the row
	...
	/**
	* \fn int image<T>::read_with_ami_bmp (std::string name)
	* \brief Read an image with ami_bmp
	* \author Jesus Rodriguez
	*/
	template <class T>
	int image<T>::read_with_ami_bmp (std::string name)
	{
		// BUILD NAME
		char name_c[200];
		snprintf(name_c, 199, "%s", name.c_str());
		...
		int width, height;
		unsigned char *red, *green, *blue;
		int output_value = ami_read_bmp(name_c, &red, &green, &blue, &width, &height);
		if (output_value != 0) return output_value;
		// ACTUALIZE INTERNAL INFORMATION OF IMAGE
		width_ = width;
		height_= height;
		nChannels_ = 3;
		// INITIALIZE INDIXES
		...
		for(long i=height_-1;i>-1;i--)
		{
			...
			{
				image_[mr] =(T)red[m];//R
				image_[mg] =(T)green[m];//G
				image_[mb] =(T)blue[m];//B
			}
		}
		// FREE MEMORY
	...
	* \brief Read an image selecting the library depend on the image format, returns 0 when it can't load the image.
	* \author Pedro Henríquez
	*/
	template <class T>
	int image<T>::read (std::string name)
	{
		return read_with_ami_bmp(name);
	}
